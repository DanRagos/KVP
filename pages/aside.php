...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
      <?php
        $patchDate = "2025-01-25";
        $todayDate = date('Y-m-d');

        // Add 5 days to the patch date
        $patchDatePlus5 = date('Y-m-d', strtotime($patchDate . ' +5 days'));

        if ($patchDatePlus5 >= $todayDate) {
        ?>
            <li class="nav-item">
                <a class="nav-link text-white <?php echo (basename($_SERVER['PHP_SELF']) == "patch.php") ? "bg-gradient-primary active" : ""; ?>" href="../pages/patch.php">
                    <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
                        <i class="material-icons opacity-10">favorite</i>
                    </div>
                    <span class="nav-link-text ms-1">Patch</span>
                </a>
            </li>
        <?php
        }
...
<a class="nav-link text-white <?php  echo (basename($_SERVER['PHP_SELF'] )== "schedule.php")?  "bg-gradient-primary active" : "";?>"  href="../pages/schedule.php">
