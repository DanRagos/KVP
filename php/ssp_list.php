...
$table = <<<EOT
 ...
 WHERE schedule.status IN (2, 3) AND (contract.isActive = 1 OR contract.isActive IS NULL) ORDER BY schedule.schedule_id DESC ) temp
EOT;
// indexes
$columns = array(
...
(SELECT schedule.schedule_id, 
       ...
       CASE 
           WHEN service_call.contract_id IS NOT NULL THEN COALESCE(contract.brand, service_call.brand) 
           ELSE COALESCE(service_call.brand, contract.brand) 
       END AS brand, 
       CASE 
           WHEN service_call.contract_id IS NOT NULL THEN COALESCE(contract.model, service_call.model) 
           ELSE COALESCE(service_call.model, contract.model) 
       END AS model,
       COALESCE(clients.client_name, CASE WHEN service_call.guest = 0 THEN service_call.guest_name END) AS client_name, 
       service_call.rep_problem, 
       machine_type.machine_name, 
       service_call.contract_id as sv_contract, 
       contract.frequency, 
...
WHERE schedule.status != 2 AND (contract.isActive = 1 OR contract.isActive IS NULL))temp
EOT;
// indexes
$columns = array(
...
array( 'db' => 'db_type', 'dt' => 'db_type'),
 ...
 else if(isset($_GET['db'])&& $_GET['db']==12){
	$primaryKey = 'contract_id';
	$isActive = 1;
	...
	// DB table to use
$table = <<<EOT
(SELECT clients.client_id as clientId, clients.client_name, clients.client_address, contract.*, machine_type.machine_name from clients inner join contract on clients.client_id = contract.client_id left join machine_type on contract.machine_type = machine_type.machine_id where contract.isActive = 0) temp
EOT;
// indexes
$columns = array(
	array( 'db' => 'contract_id', 'dt' => 'contractId'),
